...
if (isset($_POST['btn_add'])) {
    ...
    $tblresident_query = mysqli_query($con, "INSERT INTO tblresident (lname, fname, mname) values ('$lastname', '$ddl_resident', '$middlename')") or die('Error: ' . mysqli_error($con));
    ...
    if ($num_rows == 0) {
        if ($_SESSION['role'] == "Administrator") {
            ...
            values ('$txt_cnum','$residentid', '$txt_findings','$txt_purpose', '$txt_ornum', '$txt_amount', '$date', '" . $_SESSION['username'] . "','Approved')") or die('Error: ' . mysqli_error($con));
        } else {
            $query = mysqli_query($con, "INSERT INTO tblclearance (clearanceNo,residentid,findings,purpose,orNo,samount,dateRecorded,recordedBy,status) 
            values ('$txt_cnum','$residentid', '$txt_findings','$txt_purpose', '$txt_ornum', '$txt_amount', '$date', '" . $_SESSION['username'] . "','New')") or die('Error: ' . mysqli_error($con));
        }
        if ($query == true) {
            $_SESSION['added'] = 1;
